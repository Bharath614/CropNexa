...
export const OtpModal = ({ emailAddress, onVerifySuccess, onClose }) => {
    const { dispatchOutboundNotification, showToast } = useFarm();
    const [otpDigits, setOtpDigits] = useState(['', '', '', '', '', '']);
    const [generatedOtp, setGeneratedOtp] = useState('');
    const [validitySeconds, setValiditySeconds] = useState(300); // 5 minutes
    const [resendTimer, setResendTimer] = useState(30); // 30 seconds
    const [resendAttempts, setResendAttempts] = useState(0);
    const [errorMsg, setErrorMsg] = useState(null);
    const [successMsg, setSuccessMsg] = useState(null);
    const [isSendingEmail, setIsSendingEmail] = useState(false);
    const [previewEmailUrl, setPreviewEmailUrl] = useState(null);
    ...
    const generateNewOtp = async () => {
        ...
        dispatchOutboundNotification('email', emailAddress.trim(), 'CropNexa 6-Digit Email Verification Code', `Your 6-digit CropNexa registration verification OTP is: ${code}\n\nThis code is valid for 5 minutes. Do not share this code with anyone.`, 'verify_email');
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn select-none">
      <div className="bg-slate-900 border border-emerald-800 rounded-3xl max-w-lg w-full p-6 sm:p-8 space-y-6 shadow-2xl relative">
        ...
        <div className="bg-slate-950/80 border border-slate-800 p-4 rounded-2xl space-y-2.5 text-sm">
          <div className="flex items-center justify-between">
            ...
            {isSendingEmail && (<span className="text-xs text-amber-400 font-bold flex items-center gap-1">
          ...
          {previewEmailUrl && (<div className="pt-2 border-t border-slate-900 flex justify-between items-center">
              <span className="text-xs text-slate-400">Test SMTP Live Inbox:</span>
              <a href={previewEmailUrl} target="_blank" rel="noreferrer" className="text-xs text-emerald-400 hover:underline font-bold flex items-center gap-1 bg-emerald-950/80 px-2.5 py-1 rounded-xl border border-emerald-800">
        ...
        {errorMsg && (<div className="bg-rose-950/60 border border-rose-800 text-rose-300 p-3.5 rounded-2xl text-xs flex items-center gap-2.5">
            <AlertCircle className="h-4 w-4 text-rose-400 shrink-0"/>
            <span>{errorMsg}</span>
          </div>)}

        {successMsg && (<div className="bg-emerald-950/80 border border-emerald-800 text-emerald-300 p-4 rounded-2xl text-sm flex items-center gap-2.5 font-bold animate-pulse">
        ...
        <form onSubmit={handleVerify} className="space-y-6">
          <div className="flex justify-center items-center gap-2.5 sm:gap-3.5">
            {otpDigits.map((digit, idx) => (<input key={idx} id={`otp-digit-${idx}`} type="text" maxLength={1} value={digit} onChange={(e) => handleChangeDigit(idx, e.target.value)} onKeyDown={(e) => handleKeyDown(idx, e)} className="w-11 h-14 sm:w-14 sm:h-16 bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded-2xl text-center text-xl sm:text-2xl font-black text-white focus:outline-none transition-colors shadow-inner"/>))}
          </div>

          <div className="flex items-center justify-between text-xs pt-1">
            <span className="text-slate-400">Didn't receive the email code?</span>
            <button type="button" disabled={resendTimer > 0 || resendAttempts >= 3} onClick={handleResend} className={`font-bold flex items-center gap-1.5 cursor-pointer ${resendTimer > 0 || resendAttempts >= 3 ? 'text-slate-600 cursor-not-allowed' : 'text-emerald-400 hover:underline'}`}>
              <RefreshCw className={`h-4 w-4 ${resendTimer > 0 ? 'animate-spin-slow' : ''}`}/>
              {resendTimer > 0 ? `Resend in ${resendTimer}s` : 'Resend Email OTP'}
            </button>
          </div>

          <button type="submit" className="w-full py-3.5 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white font-extrabold rounded-2xl text-sm shadow-lg shadow-emerald-950/60 flex items-center justify-center gap-2 cursor-pointer transition-all hover:scale-[1.01]">
