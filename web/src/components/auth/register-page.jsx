...
export const RegisterPage = ({ onRegisterSuccess, onGoToLogin, currentLanguage, onLanguageChange, existingUsers = [] }) => {
    ...
    // Farming Info
    const [farmingMode, setFarmingMode] = useState('Organic Farming');
    ...
    const validateStep1 = () => {
        ...
        const isDuplicate = existingUsers.some(u => u.email.toLowerCase() === email.trim().toLowerCase());
        if (isDuplicate) {
    ...
    return (<div className="min-h-screen w-full bg-slate-950 text-slate-100 flex items-center justify-center p-4 sm:p-8 relative overflow-hidden py-10 text-base">
      ...
      <div className="w-full max-w-3xl bg-slate-900/90 border border-emerald-950/80 rounded-3xl p-6 sm:p-12 shadow-2xl backdrop-blur-xl space-y-7 relative z-10 animate-fadeIn my-auto">
        ...
        {isRegisteredCompleted ? (<div className="bg-emerald-950/60 border border-emerald-800 p-8 rounded-3xl text-center space-y-5 animate-fadeIn">
            ...
            <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-3.5">
              <button onClick={onGoToLogin} className="w-full sm:w-auto px-8 py-3.5 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white font-extrabold rounded-2xl text-sm shadow-xl cursor-pointer transition-all hover:scale-105">
                Go to Sign In Page
              </button>
              
              <button onClick={handleResendVerificationEmail} disabled={emailResendTimer > 0} className={`w-full sm:w-auto px-6 py-3.5 bg-slate-950 border border-slate-800 text-sm font-bold flex items-center justify-center gap-2 rounded-2xl cursor-pointer ${emailResendTimer > 0 ? 'text-slate-500 cursor-not-allowed' : 'text-emerald-400 hover:bg-slate-900'}`}>
            ...
            ].map((st) => (<div key={st.step} className={`flex items-center gap-2 font-extrabold ${currentStep === st.step ? 'text-emerald-400' : currentStep > st.step ? 'text-teal-300' : 'text-slate-500'}`}>
                  <div className={`h-7 w-7 rounded-full flex items-center justify-center text-xs font-black ${currentStep === st.step ? 'bg-emerald-500 text-slate-950 shadow' : currentStep > st.step ? 'bg-teal-900 text-teal-300' : 'bg-slate-900 text-slate-500'}`}>
                    {st.step}
                  </div>
            ...
            {/* Error Notification */}
            {errorMsg && (<div className="bg-rose-950/60 border border-rose-800 text-rose-300 p-4 rounded-2xl text-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-fadeIn">
                <div className="flex items-center gap-3">
                  <AlertCircle className="h-5 w-5 text-rose-400 shrink-0"/>
                  <span>{errorMsg}</span>
                </div>
                {(errorMsg.includes('already exists') || errorMsg.includes('sign in')) && (<button type="button" onClick={onGoToLogin} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs shrink-0 cursor-pointer shadow transition-all hover:scale-105">
                    Go to Sign In
                  </button>)}
              </div>)}
            ...
            <form onSubmit={handleRegisterSubmit} className="space-y-5">
              {currentStep === 1 && (<div className="space-y-4 animate-fadeIn">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input type="text" required value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="e.g. Rajesh Kumar" className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 text-slate-100 placeholder-slate-500 rounded-2xl pl-11 pr-4 py-3 text-sm font-medium focus:outline-none transition-colors"/>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="e.g. rajesh.kumar@cropnexa.in" className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 text-slate-100 placeholder-slate-500 rounded-2xl pl-11 pr-4 py-3 text-sm font-medium focus:outline-none transition-colors"/>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <input type={showPassword ? 'text' : 'password'} required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min 6 characters" className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 text-slate-100 placeholder-slate-500 rounded-2xl pl-11 pr-11 py-3 text-sm font-medium focus:outline-none transition-colors"/>
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-3.5 text-slate-500 hover:text-slate-300 cursor-pointer">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <Lock className="h-5 w-5 text-emerald-500 absolute left-3.5 top-3.5"/>
                        <input type={showPassword ? 'text' : 'password'} required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Re-enter password" className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 text-slate-100 placeholder-slate-500 rounded-2xl pl-11 pr-4 py-3 text-sm font-medium focus:outline-none transition-colors"/>
                      </div>
                    </div>
                  </div>
                </div>)}
              ...
              {currentStep === 2 && (<div className="space-y-4 animate-fadeIn">
                  <div className="space-y-2">
                    ...
                    <input type="text" required value={farmName} onChange={(e) => setFarmName(e.target.value)} placeholder="e.g. Green Horizon Organic Farm" className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 text-slate-100 placeholder-slate-500 rounded-2xl px-4 py-3 text-sm font-medium focus:outline-none transition-colors"/>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-slate-300 block">{t('authVillage')}</label>
                      <input type="text" required value={village} onChange={(e) => setVillage(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-slate-200"/>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-slate-300 block">{t('authDistrict')}</label>
                      <input type="text" required value={district} onChange={(e) => setDistrict(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-slate-200"/>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-slate-300 block">{t('authState')}</label>
                      <input type="text" required value={state} onChange={(e) => setState(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-slate-200"/>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 border-t border-slate-800">
                    <div className="space-y-1.5">
                      ...
                      <input type="number" step="0.1" value={totalLandArea} onChange={(e) => setTotalLandArea(parseFloat(e.target.value) || 0)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-slate-200"/>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-slate-300 block">Irrigated (Ha)</label>
                      <input type="number" step="0.1" value={irrigatedArea} onChange={(e) => setIrrigatedArea(parseFloat(e.target.value) || 0)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-slate-200"/>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-slate-300 block">Rainfed (Ha)</label>
                      <input type="number" step="0.1" value={rainfedArea} onChange={(e) => setRainfedArea(parseFloat(e.target.value) || 0)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-slate-200"/>
                    </div>
                  </div>
                </div>)}

              {currentStep === 3 && (<div className="space-y-5 animate-fadeIn">
                  <div className="space-y-2">
                    <label className="text-sm font-extrabold text-slate-200 block">{t('authFarmingMode')}</label>
                    <select value={farmingMode} onChange={(e) => setFarmingMode(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-4 py-3 text-sm text-slate-200 font-bold focus:outline-none">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <select value={primaryCrop} onChange={(e) => setPrimaryCrop(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-4 py-3 text-sm text-slate-200 font-medium">
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="text" value={secondaryCrop} onChange={(e) => setSecondaryCrop(e.target.value)} placeholder="e.g. Marigold, Cowpea, Basil" className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-4 py-3 text-sm text-slate-200"/>
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-between pt-5 border-t border-slate-800">
                {currentStep > 1 ? (<button type="button" onClick={() => setCurrentStep((prev) => (prev - 1))} className="px-5 py-2.5 bg-slate-950 hover:bg-slate-900 text-slate-300 font-extrabold rounded-xl text-sm flex items-center gap-1.5 cursor-pointer transition-colors">
                    <ArrowLeft className="h-4 w-4"/> {t('authBack')}
                  </button>) : (<button type="button" onClick={onGoToLogin} className="text-sm font-bold text-slate-400 hover:text-slate-200 cursor-pointer">
                    {t('authCancelLogin')}
                  </button>)}

                {currentStep < 3 ? (<button type="button" onClick={handleNext} className="px-6 py-3 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white font-extrabold rounded-xl text-sm flex items-center gap-2 cursor-pointer shadow-lg transition-all">
                    <span>{t('authNextStep')}</span>
                    <ArrowRight className="h-4 w-4"/>
                  </button>) : (<button type="submit" disabled={isSubmitting} className="px-7 py-3.5 bg-gradient-to-r from-emerald-500 to-teal-400 text-slate-950 font-black rounded-xl text-sm shadow-xl shadow-emerald-500/20 flex items-center gap-2 cursor-pointer transition-all hover:scale-105 disabled:opacity-50">
