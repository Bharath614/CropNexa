...
export const Reports = () => {
    ...
    const [activePreview, setActivePreview] = useState('farm');
    ...
    return (<div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fadeIn items-start">
      <div className="lg:col-span-1 bg-slate-900/60 border border-emerald-950/40 rounded-3xl p-5 shadow-md space-y-4">
        ...
        <div className="space-y-2">
          {[
            ...
            { id: 'yield', label: t('yieldForecast'), action: exportYieldReport }
        ].map((item) => (<div key={item.id} className={`
              ...
              `}>
              <button onClick={() => setActivePreview(item.id)} className="flex items-center gap-2 text-xs font-semibold text-slate-300 hover:text-slate-100 text-left">
                <FileText className="h-4.5 w-4.5 text-slate-500"/>
                {item.label}
              </button>
              
              <button onClick={item.action} className="p-1.5 bg-slate-900 hover:bg-emerald-950 text-slate-400 hover:text-emerald-400 border border-slate-800 hover:border-emerald-900 rounded-lg transition-colors cursor-pointer" title="Download CSV Spreadsheet">
                <Download className="h-4 w-4"/>
              </button>
            </div>))}
        </div>

        {downloadSuccess && (<div className="bg-emerald-950/50 border border-emerald-900/50 p-3 rounded-2xl text-[10px] text-emerald-400 flex items-center gap-2">
      ...
      <div className="lg:col-span-2 space-y-6">
        <div className="bg-slate-900/60 border border-emerald-950/40 rounded-3xl p-4 flex items-center justify-between shadow-md">
          ...
          <button onClick={handlePrint} className="flex items-center gap-1.5 py-2 px-4 bg-emerald-900 hover:bg-emerald-800 text-white rounded-xl text-xs font-bold shadow-md hover:shadow-emerald-500/20 transition-all cursor-pointer">
            <Printer className="h-4 w-4"/>
            {t('printToPdf')}
          </button>
        </div>

        <div id="print-section" className="bg-slate-950 border border-slate-900 rounded-3xl p-8 shadow-inner text-slate-350 min-h-[480px] print:p-0 print:border-0 print:bg-white print:text-black">
          ...
          {activePreview === 'farm' && (<div className="space-y-6">
              <div>
          ...
          {activePreview === 'soil' && (<div className="space-y-6">
              <div>
          ...
          {activePreview === 'weather' && (<div className="space-y-6">
              <div>
                ...
                <div className="space-y-2 text-xs">
                  {weather.forecast.map((fc, i) => (<div key={i} className="flex justify-between items-center bg-slate-900/20 p-2 border border-slate-900 rounded-xl print:border-slate-300 print:text-black">
                      <span>{fc.day}</span>
                      <span>{fc.status}</span>
            ...
            return (<div className="space-y-6">
                <div>
                  ...
                  <div className="space-y-3 text-xs mb-6">
                    {companions.length > 0 ? companions.map((comp, i) => (<div key={i} className="bg-slate-900/20 p-3 border border-slate-900 rounded-xl print:border-slate-300 print:text-black space-y-1">
                      ...
                      </div>)) : <p className="text-xs text-slate-500">No companion data available.</p>}
                  </div>

                  <h4 className="text-xs font-bold text-rose-450 uppercase mb-3">Antagonistic Crops to Avoid together</h4>
                  <div className="space-y-2 text-xs">
                    {antagonists.length > 0 ? antagonists.map((ant, i) => (<div key={i} className="flex justify-between items-center bg-slate-900/20 p-2 border border-slate-900 rounded-xl print:border-slate-300 print:text-black">
                        <span>{ant.name}</span>
                        <span className="text-rose-400 font-bold">{ant.riskCategory}</span>
                        <span>Severity Score: {ant.riskScore}/100</span>
                      </div>)) : <p className="text-xs text-slate-500">No antagonist data available.</p>}
                  </div>
                </div>
              </div>);
        })()}

          {activePreview === 'nutrient' && (<div className="space-y-6">
              <div>
          ...
          {activePreview === 'yield' && (<div className="space-y-6">
              <div>
