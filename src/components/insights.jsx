...
import { TrendingUp, DollarSign, Droplet, Sparkles, ShieldAlert, Award, ChevronRight } from 'lucide-react';
...
export const Insights = () => {
    ...
    const getDynamicYieldMetrics = () => {
        ...
        if (crop === 'rice') {
            baseYield = 5.2;
            basePricePerTonne = 350;
        }
        else if (crop === 'wheat') {
            baseYield = 3.8;
            basePricePerTonne = 320;
        }
        else if (crop === 'maize') {
            baseYield = 6.5;
            basePricePerTonne = 280;
        }
        else if (crop === 'tomato') {
            baseYield = 18.0;
            basePricePerTonne = 150;
        }
        else if (crop === 'potato') {
            baseYield = 22.0;
            basePricePerTonne = 130;
        }
        else if (crop === 'sugarcane') {
            baseYield = 70.0;
            basePricePerTonne = 45;
        }
        else if (crop === 'cotton') {
            baseYield = 2.2;
            basePricePerTonne = 850;
        }
    ...
    return (<div className="space-y-6 animate-fadeIn">
      ...
      <div className="bg-gradient-to-r from-emerald-950/30 to-slate-950 border border-emerald-950/40 rounded-3xl p-6 shadow-lg flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <button onClick={handleOpenNutrientGuide} className="flex items-center gap-1 text-[10px] font-bold text-emerald-400 hover:text-white uppercase tracking-wider transition-colors cursor-pointer bg-emerald-950/60 border border-800/80 px-3.5 py-2 rounded-xl shrink-0">
          Open Nutrient Guide
          <ChevronRight className="h-4 w-4"/>
        </button>
      </div>

      <NutrientGuideModal isOpen={isNutrientGuideOpen} onClose={() => setIsNutrientGuideOpen(false)}/>
    </div>);
};
