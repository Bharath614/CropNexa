...
export const Weather: React.FC = () => {
  ...
  const [activeChart, setActiveChart] = useState<'temp' | 'rain' | 'humidity' | 'dew'>('temp');
  ...
  const chartData = weather.hourlyTemp.map((item, index) => {
    let offsetHumid = 65;
    let offsetDew = 19;
    if (index === 0) { offsetHumid = 82; offsetDew = 21; }
    else if (index === 1) { offsetHumid = 75; offsetDew = 20; }
    else if (index === 2) { offsetHumid = 62; offsetDew = 18; }
    else if (index === 3) { offsetHumid = 60; offsetDew = 19; }
    else if (index === 4) { offsetHumid = 68; offsetDew = 20; }
    else if (index === 5) { offsetHumid = 76; offsetDew = 21; }
    else if (index === 6) { offsetHumid = 80; offsetDew = 22; }
    else if (index === 7) { offsetHumid = 85; offsetDew = 22; }
          ...
          <div className="flex flex-wrap gap-2">
            {[
              ...
              return (
                <button
                  key={tab.id}
                  onClick={() => setActiveChart(tab.id as 'temp' | 'rain' | 'humidity' | 'dew')}
                  className={`
        ...
        <div className="lg:col-span-3 bg-slate-900/60 border border-emerald-950/40 rounded-3xl p-6 shadow-md space-y-4">
          ...
          <div className="space-y-3">
            {weather.forecast.map((fc, index) => (
              <div 
                key={index}
                className="flex items-center justify-between p-3 bg-slate-950/40 hover:bg-slate-950/80 border border-slate-900 rounded-2xl transition-colors duration-150"
              >
                <div className="flex items-center gap-3">
