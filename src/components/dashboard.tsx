      ...
      <div className="bg-gradient-to-r from-slate-900 via-emerald-950/70 to-slate-900 border border-emerald-950/60 rounded-3xl p-6 shadow-xl relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3 shrink-0 relative z-10">
          <button
            ...
            className="px-4 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white font-bold rounded-2xl text-xs shadow-lg shadow-emerald-950/60 flex items-center gap-2 cursor-pointer transition-all"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900/70 border border-emerald-950/50 rounded-3xl p-6 shadow-lg space-y-4 flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-3">
              {alerts.slice(0, 2).map((alert) => (
                <div key={alert.id} className="bg-rose-950/30 border border-rose-900/50 p-3.5 rounded-2xl space-y-1">
          ...
          <div className="pt-3 border-t border-slate-900 space-y-2">
            ...
            {pendingTasks.slice(0, 2).map((evt) => (
              <div 
                key={evt.id}
                onClick={() => toggleCalendarEvent(evt.id)}
                className="flex items-center gap-2 bg-slate-950/80 p-2.5 rounded-xl border border-slate-800 text-xs text-slate-300 cursor-pointer hover:border-emerald-800"
              >
