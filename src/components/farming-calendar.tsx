...
import { useFarm, CalendarEvent } from '@/context/farm-context';
...
export const FarmingCalendar: React.FC = () => {
    ...
    const [selectedCrop, setSelectedCrop] = useState<string>(profile.currentCrop);
    const [plantingDate, setPlantingDate] = useState<string>('2026-07-01');
    const [filterStage, setFilterStage] = useState<string>('All');

    const [customTask, setCustomTask] = useState('');
    const [customCategory, setCustomCategory] = useState<'Irrigation' | 'Fertilizer' | 'Biofertilizer' | 'Pest Monitoring' | 'Harvesting'>('Irrigation');
    ...
    const generateStagePhases = (): GrowthStagePhase[] => {
        return [
            {
                id: 'germination',
                name: t('germination'),
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-emerald-950/50 pb-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            ...
                            className="px-4 py-2 bg-emerald-900 hover:bg-emerald-800 text-white rounded-xl text-xs font-bold shadow-md flex items-center gap-1.5 cursor-pointer shrink-0"
                        >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
                    <div>
                        ...
                        <select
                            ...
                            className="w-full bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 px-3.5 py-2 focus:outline-none focus:border-emerald-500"
                        >
                            {Object.values(CROP_DATABASE).map(crop => (
            ...
            {showAddForm && (
                <div className="bg-slate-900/80 border border-emerald-800 p-6 rounded-3xl space-y-4 shadow-xl animate-fadeIn">
                    ...
                    <form onSubmit={handleAddCustomTask} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div className="sm:col-span-3">
                            ...
                            <input
                                ...
                                placeholder="e.g. Inspect drip filter lines, apply bio-pesticide spray..."
                                required
                        ...
                        <div className="sm:col-span-3 flex justify-end gap-2 pt-2">
                            <button
                                ...
                                className="px-4 py-2 bg-slate-950 text-slate-400 rounded-xl text-xs font-semibold cursor-pointer"
                            >{t('cancel')}</button>
                            <button
                                type="submit"
                                className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold shadow-md cursor-pointer"
                            >
                                {t('insertTask')}
                            </button>
                        </div>
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1">
                ...
                <button
                    ...
                    className={`text-[10px] px-3 py-1 rounded-full font-bold uppercase tracking-wider border shrink-0 transition-colors cursor-pointer ${filterStage === 'All' ? 'bg-emerald-950 text-emerald-400 border-emerald-800' : 'bg-slate-900 text-slate-400 border-slate-800'}`}
                >
                    {t('allStages')} (6)
                </button>
                {stages.map(st => (
                    <button
                        ...
                        className={`text-[10px] px-3 py-1 rounded-full font-bold uppercase tracking-wider border shrink-0 transition-colors cursor-pointer ${filterStage === st.id ? 'bg-emerald-950 text-emerald-400 border-emerald-800' : 'bg-slate-900 text-slate-400 border-slate-800'}`}
                    >
                        {st.name}
                    </button>
                ))}
            ...
            <div className="space-y-6">
                {filteredStages.map((stage, idx) => (
                    <div
                        key={stage.id}
                        className={`bg-slate-900/60 border ${stage.borderColor}/40 rounded-3xl p-6 shadow-md space-y-4 relative overflow-hidden`}
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                            {stage.tasks.map((taskItem, tIdx) => (
                                <div
                                    key={tIdx}
                                    className="bg-slate-950/60 border border-slate-900 p-4 rounded-2xl flex items-start gap-3 hover:border-slate-800 transition-all"
                                >
            ...
            {calendar.length > 0 && (
                <div className="bg-slate-900/60 border border-emerald-950/40 rounded-3xl p-6 shadow-md space-y-4">
                    ...
                    <div className="space-y-2.5">
                        {calendar.map(evt => (
                            <div
                                ...
                                className={`p-3.5 border rounded-2xl cursor-pointer flex items-center justify-between gap-3 ${evt.completed ? 'bg-slate-950/20 border-slate-900 text-slate-500 opacity-60' : 'bg-slate-950/60 border-slate-900 text-slate-200'}`}
                            >
                                <div className="flex items-center gap-3">
