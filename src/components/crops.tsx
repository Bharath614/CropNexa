...
import { MASTER_CROPS, MasterCrop, getEngineRecommendations } from '@/utils/companion-engine';
import { 
  ArrowLeft, 
  Star, 
  CheckCircle2, 
...
export const Crops: React.FC = () => {
  ...
  const getCropEmoji = (name: string) => {
    ...
    if (n.includes('rice') || n.includes('wheat') || n.includes('millet') || n.includes('sesamum')) return '🌾';
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-6 animate-fadeIn pb-16 text-slate-200">
        ...
        <div className="relative h-64 md:h-80 rounded-3xl overflow-hidden shadow-2xl border border-slate-800">
          <img 
            src={`/crops/${selectedCrop.id}.jpg`} 
            alt={selectedCrop.name} 
            className="w-full h-full object-cover"
            onError={(e) => {
              (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1592417817098-8f3d6ef23a28?auto=format&fit=crop&w=1200&q=80';
            }}
          />
          ...
          <button 
            ...
            className="absolute top-4 left-4 h-10 w-10 bg-slate-900/80 backdrop-blur-md border border-slate-700 rounded-full flex items-center justify-center text-white hover:bg-slate-800 shadow-md transition-all cursor-pointer"
          >
        ...
        <div className="bg-emerald-950/40 border border-emerald-900/60 rounded-3xl p-6 shadow-xl space-y-4">
          ...
          <div className="space-y-3">
            {companions.length > 0 ? companions.map((comp, idx) => (
              <div key={idx} className="bg-slate-950/80 border border-emerald-950 p-4 rounded-2xl space-y-1.5">
              ...
              <p className="text-xs text-slate-500 italic">No specific companion pairings documented.</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-rose-950/40 border border-rose-900/60 rounded-3xl p-6 shadow-xl space-y-4">
          ...
          <div className="space-y-3">
            {avoids.length > 0 ? avoids.map((ant, idx) => (
              ...
              <p className="text-xs text-slate-500 italic">No antagonistic pairings documented.</p>
            )}
          </div>
        </div>
  ...
  // Dashboard Grid View
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-fadeIn pb-16 text-slate-200">
      <div className="bg-gradient-to-r from-slate-900 via-slate-950 to-slate-900 border border-slate-800 p-6 rounded-3xl space-y-4 shadow-2xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              type="text"
              placeholder="Search 25 master crops..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-slate-950 border border-slate-800 rounded-2xl py-2.5 pl-10 pr-4 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-emerald-500 shadow-inner"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none text-xs border-t border-slate-800/80 pt-3">
          {categories.map(cat => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-xl font-bold uppercase tracking-wider transition-all whitespace-nowrap cursor-pointer text-[10px] ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredCrops.map((crop) => (
          <div
            ...
            className="bg-slate-900/80 border border-slate-800 hover:border-emerald-500/50 rounded-3xl p-5 shadow-xl transition-all cursor-pointer space-y-4 group"
          >
