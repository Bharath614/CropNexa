...
import { useFarm, FarmingMode } from '@/context/farm-context';
import { CROP_DATABASE } from '@/utils/crop-database';
import { SUPPORTED_LANGUAGES, LanguageCode } from '@/utils/i18n';
import { Settings, Save, AlertCircle, Info, CheckCircle2, User, MapPin, Layers, Sprout, Database, Globe, MessageSquare, Mail, Server, ShieldCheck, Phone } from 'lucide-react';
...
export const SettingsPanel: React.FC = () => {
  ...
  const [farmingPractice, setFarmingPractice] = useState<FarmingMode>(profile.farmingPractice || 'Organic Farming');
  const [preferredLanguage, setPreferredLanguage] = useState<LanguageCode>(currentLanguage || 'en');
  ...
  // SMS Toggle State
  const [smsEnabled, setSmsEnabled] = useState<boolean>(currentUser?.smsNotificationsEnabled ?? true);

  // Gateway Config States
  const [emailGateway, setEmailGateway] = useState<'firebase' | 'sendgrid' | 'resend' | 'nodemailer'>('sendgrid');
  const [smsGateway, setSmsGateway] = useState<'twilio' | 'msg91' | 'fast2sms' | 'firebase'>('msg91');
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fadeIn items-start">
      <div className="lg:col-span-2 bg-slate-900/70 border border-emerald-950/50 rounded-3xl p-6 shadow-md space-y-6">
        ...
        <form onSubmit={handleSaveProfile} className="space-y-6">
          ...
          <div className="space-y-4 border-t border-slate-900 pt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  value={emailGateway}
                  onChange={(e) => setEmailGateway(e.target.value as any)}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 px-3.5 py-2"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 px-3.5 py-2 font-bold"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFarmingPractice(e.target.value as FarmingMode)}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 px-3.5 py-2"
                >
          ...
          {/* Feedback Messages */}
          {errorMsg && (
            <div className="bg-rose-950/30 border border-rose-900/50 p-4 rounded-2xl flex items-start gap-2.5 text-xs text-rose-300">
          ...
          {saveSuccess && (
            <div className="bg-emerald-950/30 border border-emerald-900/50 p-4 rounded-2xl flex items-start gap-2.5 text-xs text-emerald-300">
          ...
          <button
            type="submit"
            className="w-full py-3.5 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white rounded-xl text-xs font-extrabold shadow-md flex items-center justify-center gap-2 cursor-pointer"
          >
