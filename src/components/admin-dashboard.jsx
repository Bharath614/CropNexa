...
import { ShieldCheck, Users, Activity, AlertTriangle, Filter, Search, Download, CheckCircle2, Info, ShieldAlert, CalendarDays, FileText } from 'lucide-react';
...
export const AdminDashboard = () => {
    ...
    if (!currentUser?.isAdmin) {
        return (<div className="flex flex-col items-center justify-center h-96 bg-slate-900/50 rounded-3xl border border-rose-900/50">
    ...
    return (<div className="space-y-6 animate-fadeIn">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950/60 to-slate-900 border border-indigo-900/50 rounded-3xl p-6 shadow-lg flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <button onClick={triggerCSVDownload} className="flex items-center gap-2 px-4 py-2 bg-indigo-900/50 hover:bg-indigo-800 text-indigo-300 border border-indigo-700/50 rounded-xl text-xs font-bold transition-all shadow-md shrink-0 cursor-pointer">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 shadow-md h-80 flex flex-col">
            ...
            <div className="flex-1 w-full relative">
              {categoryData.length > 0 ? (<ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                ...
                </ResponsiveContainer>) : (<div className="absolute inset-0 flex items-center justify-center text-xs text-slate-500">No activity data available</div>)}
            </div>
            
            <div className="mt-4 flex flex-wrap gap-2 justify-center">
              {categoryData.map((entry, index) => (<div key={entry.name} className="flex items-center gap-1.5 text-[10px] text-slate-400">
        ...
        <div className="lg:col-span-2 bg-slate-900/60 border border-slate-800 rounded-3xl p-6 shadow-md flex flex-col h-[600px]">
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
            {filteredActivities.length === 0 ? (<div className="h-full flex flex-col items-center justify-center text-slate-500 space-y-3">
                <Search className="h-8 w-8 opacity-20"/>
                <span className="text-xs">No activity logs match your filters.</span>
              </div>) : (filteredActivities.map((activity) => (<div key={activity.id} className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800/80 hover:border-indigo-900/50 transition-colors">
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-3">
                    ...
                    <div className="flex items-start gap-3 w-full">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {activity.details && Object.keys(activity.details).length > 0 && (<div className="mt-2 bg-slate-900/50 p-2 rounded-lg border border-slate-800/50 text-[10px] grid grid-cols-2 sm:grid-cols-3 gap-2">
                            {Object.entries(activity.details).map(([k, v]) => (<div key={k} className="flex flex-col">
