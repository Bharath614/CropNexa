...
import { Mail, MessageSquare, CheckCircle2, X, Send, KeyRound } from 'lucide-react';
export const EmailSmsPreviewModal = ({ notifications, onClose, onVerifyEmailAction, onResetPasswordAction }) => {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn select-none">
      <div className="bg-slate-900 border border-emerald-800 rounded-3xl max-w-xl w-full p-6 space-y-5 shadow-2xl relative max-h-[85vh] flex flex-col">
        ...
        ].map(tab => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${activeTab === tab.id
        ...
        <div className="space-y-3 overflow-y-auto custom-scrollbar pr-1 flex-1">
          {filtered.length === 0 ? (<div className="text-center py-10 text-xs text-slate-500">
              No outbound notifications dispatched yet.
            </div>) : (filtered.map((item) => (<div key={item.id} className={`p-4 rounded-2xl border space-y-2 ${item.type === 'email'
                ? 'bg-slate-950/80 border-slate-800'
                : 'bg-emerald-950/20 border-emerald-900/40'}`}>
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    {item.type === 'email' ? (<span className="bg-sky-950 text-sky-400 p-1.5 rounded-lg border border-sky-900 flex items-center gap-1 font-bold text-[10px]">
                        <Mail className="h-3.5 w-3.5"/> EMAIL
                      </span>) : (<span className="bg-emerald-950 text-emerald-400 p-1.5 rounded-lg border border-emerald-900 flex items-center gap-1 font-bold text-[10px]">
                ...
                {item.actionType === 'verify_email' && (<div className="pt-1">
                    <button onClick={() => {
                ...
                {item.actionType === 'reset_password' && (<div className="pt-1">
                    <button onClick={() => {
