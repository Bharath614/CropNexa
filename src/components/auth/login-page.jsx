...
import { Sprout, Mail, Lock, Eye, EyeOff, LogIn, UserPlus, Globe, CheckCircle2, X, ShieldAlert, KeyRound, HelpCircle, Send } from 'lucide-react';
...
Empowering Smarter Farming 🌱`, 'reset_password');
    ...
    return (<div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4 sm:p-6 relative overflow-hidden text-base py-12">
      {/* Background Decor */}
      ...
      <div className="w-full max-w-lg bg-slate-900/95 border border-emerald-950/70 rounded-3xl p-6 sm:p-10 shadow-2xl backdrop-blur-md space-y-7 relative z-10 animate-fadeIn">
        ...
        {/* Notifications */}
        {errorMsg && (<div className="bg-rose-950/60 border border-rose-800/80 text-rose-300 p-4 rounded-2xl text-sm flex items-start gap-3 leading-relaxed">
            <ShieldAlert className="h-5 w-5 text-rose-400 shrink-0 mt-0.5"/>
            <span>{errorMsg}</span>
          </div>)}

        {successMsg && (<div className="bg-emerald-950/80 border border-emerald-800 text-emerald-300 p-4 rounded-2xl text-sm flex items-center gap-3 font-bold animate-pulse">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input type="email" value={identifier} onChange={(e) => setIdentifier(e.target.value)} placeholder="e.g. rajesh.kumar@cropnexa.in" className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 text-slate-100 placeholder-slate-500 rounded-2xl pl-11 pr-4 py-3 text-sm font-medium focus:outline-none transition-colors"/>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 text-slate-100 placeholder-slate-500 rounded-2xl pl-11 pr-11 py-3 text-sm font-medium focus:outline-none transition-colors"/>
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-500 hover:text-slate-300 cursor-pointer">
          ...
          <div className="flex items-center justify-between pt-1">
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} className="h-4.5 w-4.5 rounded bg-slate-950 border-slate-800 text-emerald-500 focus:ring-emerald-500 accent-emerald-500"/>
              <span className="text-xs text-slate-300 font-bold">{t('authRememberMe')}</span>
            </label>
          </div>

          <button type="submit" className="w-full py-3.5 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white font-extrabold rounded-2xl text-sm shadow-xl shadow-emerald-950/60 flex items-center justify-center gap-2 transition-all cursor-pointer hover:scale-[1.02]">
        ...
        <div className="pt-4 border-t border-slate-800/80 space-y-3 text-center">
          ...
          <div className="pt-2 border-t border-slate-950">
            ...
            <button type="button" onClick={onGoToRegister} className="text-sm font-extrabold text-emerald-400 hover:underline flex items-center justify-center gap-1.5 mx-auto mt-1 cursor-pointer">
      ...
      {isForgotPasswordOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
            ...
            <form onSubmit={handleSendResetLink} className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input type="email" required value={resetInput} onChange={(e) => setResetInput(e.target.value)} placeholder="e.g. rajesh.kumar@cropnexa.in" className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-4 py-3 text-sm text-slate-200 focus:outline-none focus:border-emerald-500"/>
                </div>
              </div>
