...
export const SplashScreen = ({ onComplete, language = 'en' }) => {
    ...
    return (<div className="fixed inset-0 z-50 flex flex-col items-center justify-between bg-gradient-to-b from-slate-950 via-emerald-950/80 to-slate-950 text-white p-6 overflow-hidden select-none">
      ...
      <div className="w-full max-w-4xl flex items-center justify-between pt-4 relative z-10">
        ...
        <button onClick={onComplete} className="flex items-center gap-1.5 bg-slate-900/80 hover:bg-slate-800 border border-slate-800 hover:border-emerald-700 text-slate-300 hover:text-white px-4 py-1.5 rounded-full text-xs font-semibold backdrop-blur-md transition-all cursor-pointer shadow-md">
      ...
      <div className="flex flex-col items-center text-center space-y-6 max-w-xl mx-auto my-auto relative z-10">
        ...
        <div className="flex flex-wrap justify-center gap-2 pt-2">
          {['Dynamic Soil Chemistry Engine', 'OCR Report Reader', '11-Language i18n', 'Split-Nutrient AI'].map((badge, idx) => (<span key={idx} className="bg-slate-900/60 border border-emerald-900/50 text-slate-300 text-[10px] px-3 py-1 rounded-xl font-bold backdrop-blur-sm">
              ✓ {badge}
            </span>))}
        </div>
      </div>
      ...
      <div className="w-full max-w-md space-y-2 pb-6 relative z-10">
        ...
        <div className="h-1.5 w-full bg-slate-900 rounded-full overflow-hidden border border-slate-800 shadow-inner">
          <div className="h-full bg-gradient-to-r from-emerald-500 to-teal-400 transition-all duration-1000 ease-linear" style={{ width: `${((4 - secondsLeft) / 4) * 100}%` }}/>
        </div>
      </div>
    </div>);
};
