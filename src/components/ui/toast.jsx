...
export const ToastContainer = ({ toasts, onDismiss }) => {
    return (<div className="fixed bottom-5 right-5 z-50 flex flex-col gap-2.5 max-w-sm w-full pointer-events-none px-4">
...
const ToastItem = ({ toast, onDismiss }) => {
    ...
    return (<div className={`
      ...
      <button onClick={() => onDismiss(toast.id)} className="text-slate-400 hover:text-white transition-colors p-0.5 focus:outline-none">
        <X className="h-4 w-4"/>
      </button>
    </div>);
};
