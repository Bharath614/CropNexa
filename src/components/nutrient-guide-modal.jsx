...
import { X, BookOpen, Sparkles, AlertTriangle, Info } from 'lucide-react';
export const NUTRIENT_GUIDE_DATA = [
    // Macronutrients
    {
        name: 'Nitrogen',
        symbol: 'N',
...
export const NutrientGuideModal = ({ isOpen, onClose }) => {
    const [selectedCategory, setSelectedCategory] = useState('All');
    const [activeNutrientName, setActiveNutrientName] = useState('Nitrogen');
    if (!isOpen)
        return null;
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-fadeIn">
      <div className="bg-slate-900 border border-emerald-900/60 rounded-3xl max-w-5xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="bg-slate-950/80 border-b border-emerald-950/80 p-5 flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-2 bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-white rounded-xl border border-slate-800 transition-colors focus:outline-none cursor-pointer">
            <X className="h-5 w-5"/>
          </button>
        </div>
        ...
        <div className="bg-slate-950/40 border-b border-slate-900 px-6 py-3 flex items-center gap-3 overflow-x-auto">
          ...
          {['All', 'Macronutrient', 'Secondary Nutrient', 'Micronutrient'].map(cat => (<button key={cat} onClick={() => setSelectedCategory(cat)} className={`
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 custom-scrollbar">
          ...
          <div className="space-y-2.5">
            ...
            return (<div key={nut.name} onClick={() => setActiveNutrientName(nut.name)} className={`
