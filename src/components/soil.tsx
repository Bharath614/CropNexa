...
import { useFarm, SoilReport } from '@/context/farm-context';
...
export const Soil: React.FC = () => {
  ...
  const [formData, setFormData] = useState<SoilReport>(soilReport);
  ...
  const getDeficiencyStatus = (nut: NutrientDisplay) => {
    const val = soilReport[nut.key];
    if (nut.key === 'ph') {
      if (val < 6.0) return { label: 'Acidic', color: 'text-amber-400 bg-amber-950/40 border-amber-900/50' };
      if (val > 7.5) return { label: 'Alkaline', color: 'text-purple-400 bg-purple-950/40 border-purple-900/50' };
      return { label: 'Optimal', color: 'text-emerald-400 bg-emerald-950/40 border-emerald-900/50' };
    }
    if (nut.key === 'ec') {
      if (val > 1.8) return { label: 'Saline High', color: 'text-rose-400 bg-rose-950/40 border-rose-900/50' };
      return { label: 'Normal', color: 'text-emerald-400 bg-emerald-950/40 border-emerald-900/50' };
    }
    if (nut.key === 'organicCarbon') {
      if (val < 0.5) return { label: 'Deficient (Low)', color: 'text-rose-400 bg-rose-950/40 border-rose-900/50' };
      if (val < 0.8) return { label: 'Moderate', color: 'text-amber-400 bg-amber-950/40 border-amber-900/50' };
      return { label: 'Optimal', color: 'text-emerald-400 bg-emerald-950/40 border-emerald-900/50' };
    }
    if (nut.key === 'nitrogen') {
      if (val < 280) return { label: 'Low (Deficient)', color: 'text-rose-400 bg-rose-950/40 border-rose-900/50' };
      return { label: 'Optimal', color: 'text-emerald-400 bg-emerald-950/40 border-emerald-900/50' };
    }
    if (nut.key === 'phosphorus') {
      if (val < 23) return { label: 'Low (Deficient)', color: 'text-rose-400 bg-rose-950/40 border-rose-900/50' };
      return { label: 'Optimal', color: 'text-emerald-400 bg-emerald-950/40 border-emerald-900/50' };
    }
    if (nut.key === 'potassium') {
      if (val < 140) return { label: 'Low (Deficient)', color: 'text-rose-400 bg-rose-950/40 border-rose-900/50' };
      return { label: 'Optimal', color: 'text-emerald-400 bg-emerald-950/40 border-emerald-900/50' };
    }

    const targetMin = parseFloat(nut.ideal.replace('> ', ''));
    if (val < targetMin) return { label: 'Deficient', color: 'text-amber-400 bg-amber-950/40 border-amber-900/50' };
    return { label: 'Optimal', color: 'text-emerald-400 bg-emerald-950/40 border-emerald-900/50' };
  };
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-emerald-950/60 to-slate-900 border border-emerald-950/60 rounded-3xl p-6 shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-emerald-950 hover:bg-emerald-900 text-emerald-300 border border-emerald-800 rounded-xl text-xs font-bold transition-all cursor-pointer shadow-md"
          >
        ...
        <div className="lg:col-span-2 bg-slate-900/70 border border-emerald-950/50 rounded-3xl p-6 shadow-lg flex flex-col justify-between space-y-4">
          ...
          {!uploadedFile ? (
            ...
            >
              ...
              {isProcessing && (
                <div className="flex items-center gap-2 text-xs text-emerald-400 font-bold bg-emerald-950/60 px-3 py-1.5 rounded-full border border-emerald-800 animate-pulse">
            ...
            <div className="bg-slate-950/80 border border-emerald-900/60 p-4 rounded-2xl space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => fileInputRef.current?.click()}
                    className="p-2 bg-slate-900 hover:bg-slate-800 text-sky-400 rounded-xl text-xs border border-slate-800"
                    title="Replace File"
                  >
                  ...
                  <button
                    ...
                    className="p-2 bg-rose-950/50 hover:bg-rose-900 text-rose-300 rounded-xl text-xs border border-rose-900/60"
                    title="Delete File"
                  >
              ...
              {uploadedFile.previewUrl ? (
                <div className="mt-2 relative h-32 w-full rounded-xl overflow-hidden border border-slate-800 bg-slate-900">
                ...
                <div className="mt-2 bg-slate-900/80 p-3 rounded-xl border border-slate-800 flex items-center justify-between text-xs text-slate-300">
      ...
      {/* Manual Entry Form */}
      {isEditingMetrics && (
        <div className="bg-slate-900/90 border border-emerald-800 p-6 rounded-3xl space-y-4 shadow-xl animate-fadeIn">
          <div className="flex items-center justify-between border-b border-emerald-950/60 pb-3">
          ...
          <form onSubmit={handleSaveManualEntry} className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {[
              ...
              <div key={param.key} className="space-y-1">
                ...
                <input
                  type="number"
                  step="0.01"
                  value={formData[param.key as keyof SoilReport]}
                  onChange={(e) => setFormData({ ...formData, [param.key]: parseFloat(e.target.value) || 0 })}
            ...
            <div className="col-span-full flex justify-end gap-3 pt-2 border-t border-slate-800">
              <button
                ...
                className="px-4 py-2 bg-slate-950 text-slate-400 rounded-xl text-xs font-semibold"
              >{t('cancel')}</button>
              <button
                type="submit"
                className="px-5 py-2 bg-gradient-to-r from-emerald-600 to-teal-500 text-white font-bold rounded-xl text-xs shadow-md flex items-center gap-1.5 cursor-pointer"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {nutrients.map((nut) => {
            ...
            return (
              <div 
                key={nut.key} 
                className="bg-slate-950/80 border border-slate-900 p-4 rounded-2xl hover:border-emerald-900/50 transition-colors space-y-2"
              >
          ...
          <div className="md:col-span-2 space-y-3 bg-slate-950/60 p-5 rounded-2xl border border-slate-900">
            ...
            <ul className="space-y-2.5 text-xs text-slate-300 pl-4 list-disc marker:text-emerald-400">
              {(profile.farmingPractice === 'Organic Farming' 
                ...
                : soilEvaluation.fertilizerRecommendations.conventional
              ).map((rec, i) => (
          ...
          <div className="bg-slate-950/60 p-5 rounded-2xl border border-slate-900 space-y-3 flex flex-col justify-between">
            <div>
              ...
              <div className="space-y-2">
                {soilEvaluation.companionRecommendations.map((sug, i) => (
                  <div key={i} className="bg-slate-900/90 p-2.5 rounded-xl border border-slate-800 text-[11px] text-emerald-300 font-bold flex items-center gap-2">
