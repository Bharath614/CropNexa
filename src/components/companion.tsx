...
import { 
  ...
  getEngineNutrientGuidance 
} from '@/utils/companion-engine';
import { 
  Check, 
  Plus, 
  AlertTriangle, 
...
export const Companion: React.FC = () => {
  ...
  // Engine State Inputs
  const [targetCrop, setTargetCrop] = useState<string>(profile.currentCrop || 'Maize');
  const [growthStage, setGrowthStage] = useState<GrowthStage>(profile.currentStage || 'Growth');
  const [farmingMode, setFarmingMode] = useState<FarmingMode>((profile.farmingPractice as FarmingMode) || 'Integrated');
  const [landAreaHa, setLandAreaHa] = useState<number>(profile.totalLandArea || 1.0);
  const [useSoilTestOverride, setUseSoilTestOverride] = useState<boolean>(true);
  const [addedCompanions, setAddedCompanions] = useState<string[]>([]);
  const [activeMainTab, setActiveMainTab] = useState<'Recommendations' | 'Nutrients'>('Recommendations');

  // Query Relational Recommendation Engine!
  const recs = getEngineRecommendations(
    targetCrop,
    growthStage,
    farmingMode,
    landAreaHa
  ...
  const nutrients = getEngineNutrientGuidance(
    targetCrop,
    farmingMode,
    useSoilTestOverride ? soilReport : undefined
  );

  // Helper for crop emojis
  const getCropEmoji = (name: string) => {
    const n = name.toLowerCase();
    if (n.includes('cowpea') || n.includes('bean') || n.includes('gram') || n.includes('lupin') || n.includes('pea')) return '🫘';
    if (n.includes('sunflower')) return '🌻';
    if (n.includes('marigold')) return '🌼';
    if (n.includes('sesame') || n.includes('mustard') || n.includes('wheat') || n.includes('sorghum') || n.includes('azolla')) return '🌾';
    if (n.includes('onion') || n.includes('garlic')) return '🧅';
    if (n.includes('cucumber') || n.includes('squash') || n.includes('nasturtium')) return '🥒';
    if (n.includes('basil') || n.includes('thyme') || n.includes('clover') || n.includes('grass')) return '🌿';
    if (n.includes('carrot') || n.includes('radish')) return '🥕';
    if (n.includes('turmeric') || n.includes('pepper') || n.includes('chili')) return '🌶️';
    return '🌱';
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 animate-fadeIn pb-16 text-slate-200">
      ...
      <div className="bg-[#111625]/90 border border-slate-800/80 rounded-3xl p-6 shadow-xl space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none bg-slate-950 border border-emerald-950 text-emerald-400 font-bold text-xs rounded-2xl px-4 py-3 pr-9 focus:outline-none focus:border-emerald-500 shadow-inner cursor-pointer"
            >
              {MASTER_CROPS.map(crop => (
                <option key={crop.id} value={crop.name}>Target Crop: {crop.name}</option>
              ))}
            </select>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs pt-2 border-t border-slate-800/80">
          <div>
            ...
            <select
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2 font-bold text-slate-300 focus:outline-none"
            >
          ...
          <div>
            ...
            <input
              ...
              step="0.5"
              value={landAreaHa}
              onChange={(e) => setLandAreaHa(parseFloat(e.target.value) || 1.0)}
              className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2 font-bold text-slate-300 focus:outline-none"
            />
          </div>
          ...
          <div>
            ...
            <button
              onClick={() => setUseSoilTestOverride(!useSoilTestOverride)}
              className={`w-full p-2 rounded-xl text-[11px] font-bold border transition-all cursor-pointer ${
                useSoilTestOverride 
        ...
        <div className="flex gap-2 text-xs pt-1">
          ...
          <button
            onClick={() => setActiveMainTab('Nutrients')}
            className={`px-4 py-2 rounded-xl font-bold transition-all cursor-pointer ${
              activeMainTab === 'Nutrients'
                ? 'bg-emerald-900 text-white border border-emerald-700 shadow-md'
                : 'bg-slate-950 text-slate-400 hover:bg-slate-900 border border-slate-800'
            }`}
          >
      ...
      {activeMainTab === 'Recommendations' && (
        <div className="space-y-6 animate-fadeIn">
          ...
          {recs.isMonocropAdvisory ? (
            <div className="bg-amber-950/40 border border-amber-800/80 p-5 rounded-3xl space-y-3 text-xs shadow-xl">
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-amber-900/60 space-y-2">
                ...
                <ul className="list-disc pl-4 text-xs text-slate-300 space-y-1">
                  {recs.rotationAdvice.map((adv, idx) => (
                    <li key={idx}>{adv}</li>
                  ))}
                </ul>
              </div>
            ...
            <div className="space-y-4">
              ...
              {recs.companions.map((comp, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="bg-[#111625]/90 border border-slate-800/80 rounded-3xl p-5 shadow-xl hover:border-emerald-500/50 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                  >
                    ...
                    <button
                      onClick={() => handleAddCompanion(comp.cropName)}
                      className={`px-6 py-2.5 rounded-xl text-xs font-bold shadow-md transition-all cursor-pointer shrink-0 self-end sm:self-center ${
                        isAdded
          ...
          <div className="bg-rose-950/40 border border-rose-900/60 rounded-3xl p-6 space-y-4 shadow-xl">
            ...
            <div className="space-y-3">
              {recs.avoids.length > 0 ? recs.avoids.map((ant, idx) => (
                <div key={idx} className="bg-slate-950/80 border border-rose-950 p-4 rounded-2xl space-y-1">
                ...
                <p className="text-xs text-slate-500 italic">No antagonistic pairings documented for {targetCrop}.</p>
              )}
            </div>
          </div>
        </div>
      ...
      {activeMainTab === 'Nutrients' && (
        <div className="bg-[#111625]/90 border border-slate-800/80 rounded-3xl p-6 shadow-xl space-y-5 animate-fadeIn">
          <div className="flex items-center justify-between border-b border-slate-800 pb-4">
            ...
            {nutrients.isSoilReportOverridden && (
              <span className="bg-emerald-950 border border-emerald-800 text-emerald-400 text-xs font-bold px-3 py-1 rounded-full">
                Soil Report Active ✓
              </span>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-900 space-y-2">
              ...
              <ul className="list-disc pl-4 text-slate-400 space-y-1">
                {nutrients.details.basalDose.map((item, idx) => (
                  <li key={idx}>{item}</li>
                ))}
              </ul>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-900 space-y-2">
              ...
              <ul className="list-disc pl-4 text-slate-400 space-y-1">
                {nutrients.details.biofertilizers.map((item, idx) => (
                  <li key={idx}>{item}</li>
                ))}
              </ul>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-900 space-y-2">
              ...
              <ul className="list-disc pl-4 text-slate-400 space-y-1">
                {nutrients.details.foliarMicronutrients.map((item, idx) => (
                  <li key={idx}>{item}</li>
                ))}
              </ul>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-900 space-y-2">
              ...
              <ul className="list-disc pl-4 text-slate-400 space-y-1">
                {nutrients.details.stageSplits.map((item, idx) => (
                  <li key={idx}>{item}</li>
                ))}
              </ul>
            </div>
