...
import { Sprout, BadgeAlert, AlertTriangle, X, ShieldAlert, LogOut, Globe, Bell } from 'lucide-react';
...
export default function Home() {
  ...
  if (authScreen === 'splash') {
    return (
      <SplashScreen 
        onComplete={() => setAuthScreen(isAuthenticated ? 'app' : 'login')} 
  ...
  if (authScreen === 'login') {
    return (
      <LoginPage 
        onLoginSubmit={(id, pass, rem) => loginUser(id, pass, rem)}
  ...
  if (authScreen === 'register') {
    return (
      <RegisterPage 
        onRegisterSuccess={async (prof, pass) => await registerUser(prof, pass)}
        onGoToLogin={() => setAuthScreen('login')}
        currentLanguage={currentLanguage}
        onLanguageChange={setLanguage}
        existingUsers={registeredUsers.map(u => ({ email: u.email, mobile: u.mobile }))}
      />
    );
  }
  ...
  return (
    <div className="flex min-h-screen bg-slate-950 text-slate-100 flex-col lg:flex-row">
          ...
          <div className="flex items-center gap-3">
            ...
            {alerts.length > 0 && (
              <button
                ...
                className="flex items-center gap-1.5 bg-rose-950/60 hover:bg-rose-900/80 border border-rose-800/80 px-3 py-1.5 rounded-full text-xs text-rose-300 font-bold transition-all duration-200 shadow-md cursor-pointer animate-pulse"
              >
            ...
            <button
              ...
              className="relative p-2 bg-slate-950 hover:bg-emerald-950/40 text-slate-400 hover:text-emerald-300 rounded-xl border border-slate-800 hover:border-emerald-900 transition-colors cursor-pointer"
              title="Notification Center"
            >
              <Bell className="h-4 w-4" />
              {notifications.filter(n => !n.read).length > 0 && (
                <span className="absolute -top-1.5 -right-1.5 h-4 w-4 bg-emerald-500 text-slate-950 font-black text-[9px] flex items-center justify-center rounded-full shadow border border-slate-900">
            ...
            <button
              onClick={logoutUser}
              className="p-2 bg-slate-950 hover:bg-rose-950/40 text-slate-400 hover:text-rose-300 rounded-xl border border-slate-800 hover:border-rose-900 transition-colors cursor-pointer"
              title="Logout"
            >
          ...
          <div className="bg-slate-900 border border-rose-950/80 rounded-3xl max-w-2xl w-full p-6 space-y-6 shadow-2xl relative">
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto custom-scrollbar pr-1">
              {alerts.map((alert) => (
                <div 
                  key={alert.id}
                  className={`border p-4 rounded-2xl space-y-2 ${getSeverityStyles(alert.severity)}`}
                >
      ...
      {isOutboundModalOpen && (
        <EmailSmsPreviewModal
          notifications={dispatchedOutbound}
          onClose={() => setIsOutboundModalOpen(false)}
          onVerifyEmailAction={(email) => verifyUserEmail(email)}
          onResetPasswordAction={(email) => {
