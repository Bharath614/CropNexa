...
export type SoilType = 'Alluvial' | 'Black' | 'Red' | 'Laterite' | 'Arid' | 'Forest/Mountain' | 'Peaty' | 'Saline';
export type SoilClassification = 'Excellent' | 'Good' | 'Moderate' | 'Poor' | 'Critical';

export interface SoilEvaluationResult {
  score: number; // 0 - 100
  classification: SoilClassification;
  microbialHealth: 'High Microbial Activity' | 'Optimal Microbial Balance' | 'Impaired Microbial Activity' | 'Critical Microbial Depletion';
  deficiencies: string[];
  excesses: string[];
...
export function evaluateSoilHealth(report: SoilReportData, _farmingMode: string = 'Organic Farming'): SoilEvaluationResult {
  let score = 0;
  ...
  if (report.sulphur >= 12) score += 5;
  else if (report.sulphur >= 8) score += 3.5;
  else if (report.sulphur >= 5) score += 2;
  else score += 0.5;

  // 8. Zinc (Max 5 pts) - Ideal: >= 0.6 ppm
  if (report.zinc >= 0.8) score += 5;
  else if (report.zinc >= 0.5) score += 3.5;
  else score += 1;

  // 9. Iron (Max 5 pts) - Ideal: >= 4.5 ppm
  if (report.iron >= 5.0) score += 5;
  else if (report.iron >= 3.5) score += 3.5;
  else score += 1;

  // 10. Boron (Max 4 pts) - Ideal: >= 0.5 ppm
  if (report.boron >= 0.5) score += 4;
  else if (report.boron >= 0.3) score += 2.5;
  else score += 0.5;

  // 11. Copper (Max 3 pts) - Ideal: >= 0.2 ppm
  if (report.copper >= 0.2) score += 3;
  else score += 1;

  // 12. Manganese (Max 3 pts) - Ideal: >= 2.0 ppm
  if (report.manganese >= 2.0) score += 3;
  else score += 1;
  ...
  let classification: SoilClassification = 'Moderate';
  ...
  // Microbial Health
  let microbialHealth: SoilEvaluationResult['microbialHealth'] = 'Optimal Microbial Balance';
  ...
  if (report.ph < 6.0) deficiencies.push(`Acidic Soil pH (${report.ph})`);
  if (report.ph > 7.5) excesses.push(`Alkaline Soil pH (${report.ph})`);

  if (report.ec > 1.8) excesses.push(`High Electrical Conductivity / Salinity (${report.ec} mS/cm)`);
  if (report.organicCarbon < 0.8) deficiencies.push(`Organic Carbon Deficient (${report.organicCarbon}%)`);

  if (report.nitrogen < 280) deficiencies.push(`Low Nitrogen (${report.nitrogen} kg/ha)`);
  else if (report.nitrogen > 560) excesses.push(`High Nitrogen (${report.nitrogen} kg/ha)`);

  if (report.phosphorus < 23) deficiencies.push(`Low Available Phosphorus (${report.phosphorus} kg/ha)`);
  if (report.potassium < 140) deficiencies.push(`Low Available Potassium (${report.potassium} kg/ha)`);

  if (report.sulphur < 10) deficiencies.push(`Sulphur Deficient (${report.sulphur} ppm)`);
  if (report.zinc < 0.6) deficiencies.push(`Zinc Deficient (${report.zinc} ppm)`);
  if (report.iron < 4.5) deficiencies.push(`Iron Deficient (${report.iron} ppm)`);
  if (report.boron < 0.5) deficiencies.push(`Boron Deficient (${report.boron} ppm)`);
  if (report.copper < 0.2) deficiencies.push(`Copper Deficient (${report.copper} ppm)`);
  if (report.manganese < 2.0) deficiencies.push(`Manganese Deficient (${report.manganese} ppm)`);

  // Fertilizer Recommendations
  const organicFerts: string[] = [];
  const inmFerts: string[] = [];
  const convFerts: string[] = [];
  ...
  if (report.nitrogen < 280) companions.push('Cowpea & Sunnhemp (Biomass & N-Fixation)');
  if (report.organicCarbon < 0.8) companions.push('Dhaincha & Velvet Bean (Organic Soil Builder)');
  if (report.ph > 7.5) companions.push('Mustard & Radish (Rhizosphere Exudate Acidifiers)');
  if (report.phosphorus < 23) companions.push('Buckwheat (Phosphorus Mobilizing Root System)');
  if (companions.length === 0) companions.push('Marigold & French Basil (Bio-pest Guard)');
