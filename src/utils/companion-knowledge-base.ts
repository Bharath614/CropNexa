...
export type ConfidenceLevel = 'High' | 'Moderate' | 'Low-Moderate' | 'Contextual';
export type SourceTag = 'PMC10331949' | 'PMC6316212' | 'ICAR' | 'TNAU' | 'FAO' | 'General Literature' | 'Commonly Practiced';
export type MechanismTag = 'Soil & Microbial Health' | 'Nutrient Optimization' | 'Pest & Disease Control' | 'Space Utilization';
...
export interface CompanionDetail {
  ...
  category: 'Nitrogen Fixer' | 'Pollinator Plant' | 'Pest Repellent' | 'Trap Crop' | 'Cover Crop' | 'Aromatic' | 'Medicinal' | 'Shade Plant' | 'Vegetables' | 'Cereals' | 'Spices';
  image: string;
  baseCompatibilityScore: number;
  recommendationLevel: 'Highly Recommended' | 'Recommended' | 'Moderate' | 'Avoid';
  ...
  // Mechanistic details
  explanation: string;
  rootInteraction: string;
  nutrientSharing: string;
  nitrogenFixationKgPerHa: number;
  pollinatorAttraction: string;
  pestRepellentEffect: string;
  weedSuppression: string;
  soilMoistureConservation: string;
  microbialImprovement: string;
  enzymeActivityImprovement: string;
  diseaseSuppression: string;
  yieldImprovementPercent: number;
  economicBenefitPerHa: number;
  ...
  objectives: {
    soilMicrobialHealth: { score: number; details: string };
    nutrientOptimization: { score: number; details: string };
    pestDiseaseControl: { score: number; details: string };
    spaceUtilization: { score: number; details: string };
  };

  pestsControlled: string[];
  diseasesSuppressed: string[];
  pollinatorsAttracted: string[];
  beneficialInsects: string[];
  microbialHealthBoost: string[];
  
  references: ResearchReference[];
}

export interface AvoidDetail {
  id: string;
  commonName: string;
  scientificName: string;
  botanicalFamily: string;
  ...
  waterCompetition: string;
  nutrientCompetition: string;
  rootCompetition: string;
  allelopathyEffect: string;
  sharedDiseases: string[];
  sharedPests: string[];
  yieldReductionPercent: number;
  economicLossPerHa: number;
  
  references: ResearchReference[];
}

export interface FactorBreakdown {
  nutrientCompatibility: number;
  rootCompatibility: number;
  waterCompatibility: number;
  climateCompatibility: number;
  temperatureCompatibility: number;
  rainfallCompatibility: number;
  diseaseCompatibility: number;
  pestCompatibility: number;
  growthDurationCompatibility: number;
  harvestCompatibility: number;
}

export interface CompatibilityAnalysis {
  overallScore: number;
  level: 'Excellent (90–100%)' | 'Good (75–89%)' | 'Moderate (60–74%)' | 'Avoid (Below 60%)';
  badgeStyle: string;
  color: string;
...
export interface EconomicImpact {
  ...
  fertilizerSaving: number;
  waterSaving: number;
  pesticideSaving: number;
  diseaseManagementSaving: number;
...
// ============================================================================
export const MASTER_AVOID_REGISTRY: { cropA: string; cropB: string; reason: string; confidence: ConfidenceLevel; sourceTag: SourceTag }[] = [
...
export function isAvoidedCombination(cropA: string, cropB: string): { isAvoided: boolean; reason?: string; confidence?: ConfidenceLevel } {
...
function createCompanion(config: Partial<CompanionDetail> & { commonName: string; scientificName: string }): CompanionDetail {
...
// ============================================================================
export const COMPANION_KNOWLEDGE_BASE: Record<string, { companions: CompanionDetail[]; avoids: AvoidDetail[] }> = {
  wheat: {
    companions: [
      createCompanion({
...
export function getCompanionDataForCrop(cropName: string): { companions: CompanionDetail[]; avoids: AvoidDetail[] } {
  ...
  const defaultAvoids: AvoidDetail[] = [
    {
      id: 'potato_avoid',
      commonName: 'Potato',
...
export function calculate10FactorCompatibility(
  targetCrop: string,
  ...
  let level: 'Excellent (90–100%)' | 'Good (75–89%)' | 'Moderate (60–74%)' | 'Avoid (Below 60%)' = 'Good (75–89%)';
...
export function calculateEconomicImpact(comp: CompanionDetail, landAreaHa: number = 1.0): EconomicImpact {
...
export function calculateAiRiskAnalysis(comp: CompanionDetail, weather: any): RiskAnalysis {
