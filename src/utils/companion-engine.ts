...
export type CropCategory = 'cereal' | 'millet' | 'oilseed' | 'fiber' | 'plantation' | 'spice' | 'fruit' | 'vegetable' | 'sugar';
export type LightRequirement = 'full' | 'partial' | 'shade';
export type DaylengthSensitivity = 'short-day' | 'long-day' | 'day-neutral';
export type ConfidenceLevel = 'High' | 'Moderate' | 'Low' | 'Low-Moderate' | 'Contextual';
export type FarmingMode = 'Conventional' | 'Organic' | 'Integrated';
...
export interface MasterCrop {
  ...
  daylength_sensitivity: DaylengthSensitivity;
...
export interface SeedAvoidPair {
  id: number;
  combination: string[];
  reason: string;
  confidence: ConfidenceLevel;
...
export interface RecommendationResult {
  isMonocropAdvisory: boolean;
  targetCrop: MasterCrop;
  companions: {
    cropName: string;
    mechanism: string;
    source: string;
    confidence: ConfidenceLevel;
    stageSuitability: 'Suitable' | 'Caution at Germination';
    mechanismTag: 'Soil' | 'Nutrient' | 'Pest' | 'Space';
  }[];
  avoids: {
    cropName: string;
    reason: string;
...
export interface NutrientGuidanceResult {
  mode: FarmingMode;
  isSoilReportOverridden: boolean;
  npkSummary: string;
  details: {
    basalDose: string[];
...
export const SEED_COMPANIONS: SeedCompanion[] = [
...
// 3. SEED AVOID PAIRS
export const SEED_AVOID_PAIRS: SeedAvoidPair[] = [
...
export async function queryFirestoreCompanions(cropName: string): Promise<SeedCompanion[]> {
  try {
    const q = query(
      collection(db, "companions"),
      where("combination", "array-contains", cropName)
    );
    const snap = await getDocs(q);
    if (!snap.empty) {
      return snap.docs.map(doc => ({ id: doc.id as any, ...doc.data() } as SeedCompanion));
    }
  } catch (e) {
...
export async function queryFirestoreAvoidPairs(cropName: string): Promise<SeedAvoidPair[]> {
  try {
    const avoidQ = query(
      collection(db, "avoidPairs"),
      where("combination", "array-contains", cropName)
    );
    const snap = await getDocs(avoidQ);
    if (!snap.empty) {
      return snap.docs.map(doc => ({ id: doc.id as any, ...doc.data() } as SeedAvoidPair));
    }
  } catch (e) {
...
export async function saveUserFarmProfile(uid: string, profileData: any): Promise<void> {
  if (!uid) return;
...
export function subscribeUserProfile(uid: string, callback: (data: any) => void) {
  if (!uid) return () => {};
  const userRef = doc(db, 'users', uid);
...
export function isPairInAvoidTable(cropA: string, cropB: string): { isAvoided: boolean; reason?: string; confidence?: ConfidenceLevel } {
...
export function getEngineRecommendations(
  ...
  mode: FarmingMode = 'Integrated',
  landAreaHa: number = 1.0
  ...
  const avoidList: { cropName: string; reason: string; confidence: ConfidenceLevel }[] = [];
  ...
  const companionList: RecommendationResult['companions'] = [];
  ...
  if (!isMonocropAdvisory) {
        ...
        comp.combination.forEach(item => {
          ...
          if (!cNameLower.includes(itemLower) && !itemLower.includes(cNameLower)) {
            ...
            if (!avoidCheck.isAvoided) {
              let mechTag: 'Soil' | 'Nutrient' | 'Pest' | 'Space' = 'Nutrient';
              const m = comp.mechanism.toLowerCase();
              if (m.includes('soil')) mechTag = 'Soil';
              else if (m.includes('pest') || m.includes('trap')) mechTag = 'Pest';
              else if (m.includes('space') || m.includes('shade')) mechTag = 'Space';
    ...
    if (companionList.length === 0) {
      const defaultPartners = [
        { cropName: 'Cowpea', mechanism: 'Nitrogen fixation & soil microbial enhancement', source: 'PMC10331949', confidence: 'High' as ConfidenceLevel, mechanismTag: 'Nutrient' as const },
        { cropName: 'Marigold', mechanism: 'Nematode control & natural pest repellent', source: 'PMC6316212', confidence: 'High' as ConfidenceLevel, mechanismTag: 'Pest' as const },
        { cropName: 'Sunflower', mechanism: 'Pollinator attraction & solar radiation harvesting', source: 'PMC6316212', confidence: 'Moderate' as ConfidenceLevel, mechanismTag: 'Pest' as const }
      ];
...
export function getEngineNutrientGuidance(
  cropName: string,
  mode: FarmingMode,
  soilReport?: any
): NutrientGuidanceResult {
